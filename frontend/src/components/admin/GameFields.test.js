...
describe("<GameFields />", () => {
  ...
  describe('creating a game', () => {
    ...
    it("should have an empty target title field", () => {
      const input = comp.find(TextField).filterWhere((i) => i.props().label == 'Title')
      expect(input.getElement()).not.toBeNull()
      expect(input.props().defaultValue).toEqual('')
    })

    it("should have an empty code field", () => {
      const input = comp.find(TextField).filterWhere((i) => i.props().label == 'Code')
      expect(input.getElement()).not.toBeNull()
      expect(input.props().defaultValue).toEqual(null)
    })

    it("should have an empty questions field", () => {
      const input = comp.find(TextField).filterWhere((i) => i.props().label == 'Questions JSON')
      expect(input.getElement()).not.toBeNull()
      expect(input.props().defaultValue).toEqual('[]')
    })

    it("should have an empty options field", () => {
      const input = comp.find(TextField).filterWhere((i) => i.props().label == 'Options JSON')
      expect(input.getElement()).not.toBeNull()
      expect(input.props().defaultValue).toEqual('[]')
    })
    ...
    it("should have a submit button", () => {
      let found = false
      const input = comp.find(Button).filterWhere((i) => i.prop('children') == 'Cancel')
      expect(input.getElement()).not.toBeNull()
    })

    it("should have a submit button", () => {
      let found = false
      const input = comp.find(Button).filterWhere((i) => i.prop('children') == 'Submit')
      expect(input.getElement()).not.toBeNull()
    })
  })
  ...
  describe('edting a game', () => {
    ...
    it("should have an empty questions field", () => {
      const input = comp.find(TextField).filterWhere((i) => i.props().label == 'Questions JSON')
      expect(input.getElement()).not.toBeNull()
      expect(input.props().defaultValue).toEqual(JSON.stringify(game.questions))
    })
